...
const Bankstate = (props) => {
  ...
  const allTransactionsOfUser = async () => {

    const response = await fetch(`${host}/api/transactions/fetchtransactions`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
  ...
  return (
    <Bankcontext.Provider value={{ entries, transfers, allcustomers, alltransfers, makeatransaction, showalert, alert,allTransactionsOfUser,transactionsList }}>
      {props.children}
